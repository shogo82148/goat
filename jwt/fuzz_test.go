...
func FuzzJWT_octKey(f *testing.F) {
	mockTime(f, func() time.Time {
		return time.Unix(1300819379, 0)
	})

	for _, data := range jwtPayloads {
		f.Add(data)
	}
	...
	f.Fuzz(NewFuzzJWTFunc(key))
}

func FuzzJWT_RSAKey(f *testing.F) {
	mockTime(f, func() time.Time {
		return time.Unix(1300819379, 0)
	})
...
func FuzzJWT_ECKey(f *testing.F) {
	mockTime(f, func() time.Time {
		return time.Unix(1300819379, 0)
	})
